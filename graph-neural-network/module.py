...
class GnnModule:
    ...
    @staticmethod
    def build_model(rng, model, x, G):
        init_rng, rng = jax.random.split(rng, 2)
        return model.init(init_rng, x, G)['params']
...
#                optax.clip_by_global_norm(1.0),
    ...
    def build_train_step_fn() -> Callable:
        def train_step(
            ...
            _, msk, x, G, y = batch
            loss_fn = lambda params: state.apply_fn({'params': params}, x, G)
            (loss, rng), grads = jax.value_and_grad(loss_fn)(state.params)
